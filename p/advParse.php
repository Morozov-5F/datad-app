<?php
	...
	preg_match('#<span itemprop="streetAddress">(.*?)</span>#', $page, $addressArr);
	...
	if (!$addressArr) { $address = 1; }
	else {
		$addressRow = selectrow("SELECT * FROM `address` WHERE `address` LIKE '".$addressArr[1]."' LIMIT 1");
		if (!$addressRow) 
		{ 
			echo 'Address не найден<br/>Добавляем Address: '.$addressArr[1].'<br/>';
			$address = insert('address', ['address' => $addressArr[1]]);
		}
		else {
	...
	foreach($images[1] as $i=>$img) {
		if ($i == 0) continue;
		$img_str .= substr($img,(strrpos($img, '/')) + 1, -4).($i == count($images[1]) - 1 ? '' : ', ');
	}
	...
	if (!$page['image64']) { 
 	...
 	if ($phone != -1) {
		...
		if ($phone != -2) { 
			...
			echo "Номер распознан: ".$phone.'<br/><br/>'; 
 	...
 	$data = [
		'id' => $id[1],
		'date' => strtotime(str_replace('.16', '.2016', $date_str)),
		'name_id' => $name,
		'price' => $price[1],
		'phone_code_id' => $phone_id,
		'phone' => $phone_str,
		'owner_id' => $owner,
		'region' => $region[1],
		'address_id' => $address,
